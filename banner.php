...
<?php
	...
	*/
    ...
    try {
        ...
        if (!file_exists('hsn.php' )) {
            throw new Exception ('hsn.php does not exist');
        } else {
            require_once('hsn.php' ); 
        }
        if (!file_exists('cache/packages.json') || filemtime('cache/packages.json') < filemtime('hsn.php')) {
            $packages = [];
